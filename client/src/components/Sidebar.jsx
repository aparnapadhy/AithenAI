...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      {/* New chat button*/}
      <button onClick={createNewChat} className='flex justify-center items-center w-full py-2 mt-7 text-white bg-gradient-to-r from-[#34D399] to-[#10B981] text-sm rounded-md cursor-pointer'>
        <span className='mr-2 text-xl'>+</span>New Chat
      </button>

      {/* Search conversations */}
      <div className='flex items-center gap-2 p-3 mt-1 border border-gray-400 dark-border-white/20 rounded-md'>
          ...
          chats.filter((chat) => {
            ...
            <div onClick={() => { navigate('/'); setSelectedChat(chat); setIsMenuOpen(false) }} key={chat._id} className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'>
              <div>
                ...
                <p className='text-xs text-gray-500 dark:-text-[#B1A630]'>{moment(chat.updatedAt).fromNow()}</p>
              </div>
      ...
      <div onClick={() => { navigate('/credits'); setIsMenuOpen(false) }} className='flex items-center gap-2 p-3 mt-1 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
      ...
      {/* Dark mode toggle */}
      <div className='flex justify-between items-center gap-2 p-3 mt-1 border border-gray-300 dark:border-white/15 rounded-md '>
